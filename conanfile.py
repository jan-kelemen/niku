...
class NikuConan(ConanFile):
    ...
    def requirements(self):
        ...
        self.requires("boost/1.91.0", transitive_headers=False)
        ...
        self.requires("glslang/1.4.350.0", transitive_headers=False)
        self.requires("harfbuzz/14.2.0")
        ...
        self.requires("spirv-cross/1.4.350.0", transitive_headers=False)
        self.requires("stb/cci.20240531", transitive_headers=False)
        self.requires("tree-sitter/0.26.9")
        self.requires("volk/1.4.350.0")
        self.requires("vulkan-memory-allocator/3.3.0")
        self.requires("vulkan-utility-libraries/1.4.350.0")

        if self.options.develop:
            self.requires("recastnavigation/1.6.0")
            self.requires("simdjson/4.6.4")
