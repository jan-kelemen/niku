...
class NikuConan(ConanFile):
    ...
    def requirements(self):
        ...
        self.requires("glslang/1.4.328.1", transitive_headers=False)
        self.requires("harfbuzz/12.1.0")
        self.requires("imgui/1.92.4-docking")
        self.requires("joltphysics/5.4.0")
        self.requires("mikktspace/cci.20200325", transitive_headers=False)
        self.requires("meshoptimizer/0.25", transitive_headers=False)
        self.requires("sdl/3.2.24")
        self.requires("spdlog/1.16.0", transitive_headers=False)
        self.requires("spirv-cross/1.4.328.1", transitive_headers=False)
        self.requires("stb/cci.20240531", transitive_headers=False)
        self.requires("tree-sitter/0.25.10")
        self.requires("volk/1.4.328.1")
