...
heatx::postprocess_shader_t::postprocess_shader_t(vkrndr::backend_t& backend)
    : backend_{&backend}
...
heatx::postprocess_shader_t::~postprocess_shader_t()
{
    for (auto& data : cppext::as_span(frame_data_))
    {
...
void heatx::postprocess_shader_t::draw(VkCommandBuffer command_buffer,
...
void heatx::postprocess_shader_t::resize(uint32_t width,
    uint32_t height,
